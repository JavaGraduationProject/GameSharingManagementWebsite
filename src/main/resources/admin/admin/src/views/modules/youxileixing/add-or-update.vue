<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="游戏类型" prop="youxileixing">
          <el-input v-model="ruleForm.youxileixing" 
              placeholder="游戏类型" clearable  :readonly="ro.youxileixing"></el-input>
        </el-form-item>
        <div v-else>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `youxileixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.youxileixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
