<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.youxifengmian" label="游戏封面" prop="youxifengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.youxifengmian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="发行商" prop="faxingshang">
          <el-input v-model="ruleForm.faxingshang" 
              placeholder="发行商" clearable  :readonly="ro.faxingshang"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="发行商" prop="faxingshang">
              <el-input v-model="ruleForm.faxingshang" 
                placeholder="发行商" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.youxishipin" label="游戏视频" prop="youxishipin">
          <file-upload
          tip="点击上传游戏视频"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.youxishipin" label="游戏视频" prop="youxishipin">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.youxishipin)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.biaoqian = true;
          ...
          if(o=='faxingshang'){
            this.ruleForm.faxingshang = obj[o];
	    this.ro.faxingshang = true;
            continue;
          }
          if(o=='youxishipin'){
	...
	this.ruleForm.youxigonglve = this.ruleForm.youxigonglve.replace(reg,'../../../springboot49c2c/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `youxiwenzhang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.youxiwenzhangCrossAddOrUpdateFlag = false;
						 this.parent.search();
